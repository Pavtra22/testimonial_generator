...
import { Download, Lock, Unlock, Crop, Settings, FileText } from 'lucide-react';
...
export default function ControlSidebar({ size, setSize, config, setConfig, onDownload }) {
  const updateDimension = (dim, value) => {
    ...
    if (size.locked) {
      ...
      if (dim === 'w') setSize({ ...size, w: num, h: num / aspectRatio });
      else setSize({ ...size, w: num * aspectRatio, h: num });
    } else {
  ...
  const handlePreset = (e) => {
    ...
    if (preset) setSize({ ...size, w: preset.w, h: preset.h, unit: 'px', locked: false });
  };
